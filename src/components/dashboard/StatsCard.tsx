...
export function StatsCard({ title, value, icon: Icon, change, changeType = 'neutral', color }: StatsCardProps) {
    ...
    return (
        <div className={cn(
            "bg-white border-2 border-dashed rounded-lg p-6 transition-all duration-300 group hover:bg-gray-50/50",
            colors.border
        )}>
            <div className="flex items-start justify-between mb-4">
                <div className={cn('p-3 rounded-lg border border-dashed transition-transform group-hover:scale-110 duration-300', colors.bg, colors.border)}>
                    <Icon className={cn('w-5 h-5', colors.icon)} />
                </div>
                {change && (
                    <div className={cn(
                        "flex items-center gap-1 text-[10px] font-bold uppercase tracking-widest px-2 py-1 rounded border border-dashed",
