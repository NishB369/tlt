    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-black text-gray-900 tracking-tight">Notes</h1>
                    <p className="text-gray-500 font-medium">Manage study notes and analysis points</p>
                </div>
                <Link
                    href="/admin/note/new"
            ...
            {filteredNotes.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 text-center bg-gray-50 rounded-xl border-2 border-dashed border-gray-200">
                    ...
                    <h3 className="text-xl font-black text-gray-900 mb-2">No notes found</h3>
                    <p className="text-gray-500 max-w-sm mb-6 font-medium">Get started by creating your first note.</p>
                    <Link
                        href="/admin/note/new"
                        ...
                        <table className="w-full text-left">
                            <thead>
                                <tr className="border-b border-gray-100 bg-gray-50/50">
                                    ...
                                    <th className="px-6 py-4 text-xs font-black uppercase text-gray-400 tracking-wider">Tags</th>
                                    ...
                                    <tr key={note._id || note.id} className="group hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold border ${note.isPublished ? 'bg-green-50 text-green-700 border-green-200' : 'bg-gray-100 text-gray-600 border-gray-200'}`}>
                                                <span className={`w-1.5 h-1.5 rounded-full ${note.isPublished ? 'bg-green-500' : 'bg-gray-400'}`} />
                    ...
                    {filteredNotes.map((note) => (
                        <div key={note._id || note.id} className="group relative bg-white rounded-xl border-2 border-dashed border-gray-200 overflow-hidden hover:border-gray-400 transition-all hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,0.1)] p-6 space-y-4">
                            <div className="flex items-start justify-between">
                                ...
                                <span className={`px-2 py-1 rounded-md text-[10px] font-black uppercase tracking-wider ${note.isPublished ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white rounded-xl max-w-sm w-full p-6 shadow-2xl space-y-6 animate-in zoom-in-95 duration-200">
                        <div className="text-center space-y-2">
                            ...
                            <h3 className="text-lg font-black text-gray-900">Delete Note?</h3>
